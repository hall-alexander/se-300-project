	...
	public void paintComponent(Graphics g) {
		...
		for (Point p : points) {
			...
			if (p.getConnected() == true) {
				...
				/*
				 * while (doPrevPoint = true){ g.drawLine(prevPoint.getX()+5,
				 * prevPoint.getY()+5, p.getX()+5, p.getY()+5); }
				 */
				prevPoint = p;
				doPrevPoint = true;
			}
		...
		for (Point p : points) {
			...
			if (p.getX() > firstPoint.getX() - 20 && p.getX() < firstPoint.getX() + 20
					&& p.getY() > firstPoint.getY() - 20 && p.getY() < firstPoint.getY() + 20) {
